   ...
   for (;;) {
	  ...
	  /* Print the name of the file */
	   if (event->len)
           ...
           if ((event->mask & IN_OPEN) && Q["open"].size())
	   {
	   	string tmp = Q["open"].front();
		Q["open"].pop_front();
		cout << tmp << "[" << isdir << " open]" << " " << path << event_name << endl;
	   }
           if ((event->mask & IN_CLOSE) && Q["close"].size())
           {
		string tmp = Q["close"].front();
                Q["close"].pop_front();
                cout << tmp << "[" << isdir << " close]" << " " << path << event_name  << endl;
	   }    

           if ((event->mask & IN_ACCESS) && Q["access"].size())
           {
                string tmp = Q["access"].front();
                Q["access"].pop_front();
                cout << tmp << "[" << isdir << " access]" << " " << path << event_name  << endl;
	   }

	 
           if ((event->mask & IN_MODIFY) && Q["modify"].size())
           {
                string tmp = Q["modify"].front();
                Q["modify"].pop_front();
                cout << tmp << "[" << isdir << " modify]" << " " << path << event_name  << endl;
 
	   }

           if ((event->mask & IN_MOVED_FROM) && Q["moved_from"].size())
           {
                string tmp = Q["moved_from"].front();
                Q["moved_from"].pop_front();
                cout << tmp << "[" << isdir << " moved_from]" << " " << path << event_name  << endl;
	   }    

           if ((event->mask & IN_MOVED_TO) && Q["moved_to"].size())
           {
                string tmp = Q["moved_to"].front();
                Q["moved_to"].pop_front();
                cout << tmp << "[" << isdir << " moved_to]" << " " << path << event_name  << endl;

	   }
           if ((event->mask & IN_CREATE) && Q["create"].size())
           {
                string tmp = Q["create"].front();
                Q["create"].pop_front();
                cout << tmp << "[" << isdir << " create]" << " " << path << event_name  << endl;
	  }    

           if ((event->mask & IN_DELETE) && Q["delete"].size())
           {
                string tmp = Q["delete"].front();
                Q["delete"].pop_front();
                cout << tmp << "[" << isdir << " delete]" << " " << path << event_name  << endl;
	   }    
...
static char * get_file_path_from_fd(int fd, char *buffer, size_t  buffer_size)
{
    ...
    if ((len = readlink (buffer, buffer, buffer_size - 1)) < 0) return NULL;
...
static void event_process (struct fanotify_event_metadata *event)
{
    ...
    char ctime[50];
    get_time(ctime);
    sprintf(pid, "%s %d (%s): ", ctime, (int)event->pid, (get_program_name_from_pid(event->pid, path, PATH_MAX) ? path : "unknown"));
    string tmp = pid;
    
    if (event->mask & FAN_OPEN){
	//printf("FAN_OPEN\n");
	Q["open"].push_back(tmp);
    }
    if (event->mask & FAN_CLOSE) 
    {
	Q["close"].push_back(tmp);
        //printf("FAN_CLOSE\n");
    }
    if (event->mask & FAN_ACCESS) 
    {
	Q["access"].push_back(tmp);
        //printf("FAN_ACCESS\n");
    }
    if (event->mask & FAN_MODIFY) 
    {
	Q["modify"].push_back(tmp);
        //printf("FAN_MODIFY\n");
    }
    if (event->mask & FAN_MOVED_FROM) 
    {
	Q["moved_from"].push_back(tmp);
        //printf("FAN_MOVED_FROM\n");
    }
    if (event->mask & FAN_MOVED_TO) 
    {
	Q["moved_to"].push_back(tmp);
	//printf("FAN_MOVED_TO\n");
    }
    if (event->mask & FAN_CREATE) 
    {
	Q["create"].push_back(tmp);
	//printf("FAN_CREATE\n");
    }
    if (event->mask & FAN_DELETE) 
    {
	Q["delete"].push_back(tmp);
	//printf("FAN_DELETE\n");
    }
...
int main (int argc, const char **argv)
{
    ...
    if (ifd == -1) {
     ...
     for (int i = 1; i < argc; i++) {
         wd[i] = inotify_add_watch(ifd, argv[i], IN_OPEN | IN_CLOSE | IN_MOVE | IN_CREATE | IN_DELETE | IN_MODIFY | IN_ACCESS | IN_ATTRIB);
         if (wd[i] == -1) {
    ...
    for (;;)
    {
		...
		handle_events(ifd, wd, argc, argv); 
          }
        }
    }
